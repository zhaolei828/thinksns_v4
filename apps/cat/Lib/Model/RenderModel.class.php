...
class RenderModel extends Model
{
    public function renderInfo($info_id)
    {
        ...
        switch ($tpl) {
            case -1:
                ...
                $tpl_html=W('DefaultInfoLine',array('info'=>$info),true);
                break;
            case 0: //解析预置模板
    ...
    {
        $info['data']=D('Data')->getByInfoId($info['info_id']);
    ...
    public function handleSysTags($tpl_section, $v)
    {
        ...
        $tpl_section = W('SysTagRender', array('tpl' => $tpl_section, 'info' => $v), true);
        return $tpl_section;
    }
    ...
    public function replaceFieldData($str, $data)
    {
        ...
        foreach ($data['data'] as $v) {
            $str = W('FieldRender', array('str' => $str, 'field' => $v), true);
        }
