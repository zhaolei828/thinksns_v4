...
//动态加载iscroll
/*if($.os.ios){
    $.getScript(_static+'/js/iscroll-lite.js');
}*/
$.ui = {
    cache:{
    ...
    show:function(el,transformation){
        var loadFunc = {
            ...
            footer:function(){
                if(el.find('footer').length===1 && el.data('footer')!=='none'){
                    if($.ui.cache.footerCache !== $(el.find('footer')).html()){
                        $.ui.cache.footerCache = $(el.find('footer')).html();
                        $('#footer').html($.ui.cache.footerCache);
                        $('#footer').show();
                    }
                    ...
                    if($('#content').find('footer#'+String(el.data('footer'))).length===1){
                        if($.ui.cache.footerCache !== $($('#content').find('footer#'+String(el.data('footer')))).html()){
                            $.ui.cache.footerCache = $($('#content').find('footer#'+String(el.data('footer')))).html();
                            $('#footer').html($.ui.cache.footerCache);
                            $('#footer').show();
                        }
                    }else{
                        $.ui.cache.footerCache = undefined;
                        $('#footer').html('');
                        $('#footer').hide();
                    }
                }else{
            ...
            menu:function(){
                ...
                $.ui.cache.activeContentDiv=el.attr('id');
            }
        }
        /**
        ...
        * */
        ...
        if(el.data('scrolly') && el.data('rememberScroll')===true){
...
//                el.addClass('defer-loading');
                el.prepend('<div class="loader"></div>');
                $.get(deferUrl,success);
        ...
        if($.os.ios){
            console.log(el);
//            $.ui.cache.panelScroll = new IScroll(el.get(0));
        }
     },
...
//                $('#header,#main,#footer').removeClass('blur');
    ...
    var initPanelId;
    if($.os.ios){
        /*initPanelId = setInterval(function(){
            if(typeof IScroll !== 'undefined'){
                $.ui.showDeafultContentDiv();
                clearInterval(initPanelId);
            }
        },100);*/
        $.ui.showDeafultContentDiv();
    }else{
...
$(document).ready(function(){
    ...
    //read message
    //TS.app.weibo.func.readNewMsg(true);
    TS.cache.readMsg = setInterval(function(){TS.app.weibo.func.readNewMsg()},30*1000);
    //resize #content
